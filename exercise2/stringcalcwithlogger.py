...
class StringCalculator():
    ...
    def add(self, inp):
        ...
        if not numbers:
            value = ''
            logger.info("add({0},The sum {1})".format(inp ,value))
            return value

        neg_numbers = [n for n in numbers if n < 0]
        if neg_numbers:
            logger.critical("add({0},Exception {1})".format(inp, NegativeInputException(neg_numbers)))
            raise NegativeInputException(neg_numbers)
        value = sum([n for n in numbers if n <= 1000])
        logger.info("add({0},The sum {1})".format(inp, value))
        return value
